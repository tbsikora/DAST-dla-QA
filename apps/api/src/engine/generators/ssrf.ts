...
const SSRF_PAYLOADS = [
  "http://127.0.0.1:80/tomsec-ssrf",
  "http://169.254.169.254/latest/meta-data/"
];

const URL_LIKE_PARAM = /(url|uri|link|redirect|next|target|callback|webhook|endpoint|fetch|resource)/i;

export function generateSsrfTests(
  ...
  const payloads = SSRF_PAYLOADS.slice(0, maxPayloads);
