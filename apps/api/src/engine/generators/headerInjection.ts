...
export type HeaderInjectionTest = {
  method: string;
  path: string;
  payload: string;
...
const HEADER_PAYLOADS = [
  "%0d%0aX-TomSec-Injection:%201",
  "normal%0d%0aSet-Cookie:%20tomsec=1"
];

const HEADER_LIKE_PARAM = /(header|name|filename|input|value|redirect|next|url|path|return)/i;

export function generateHeaderInjectionTests(
  endpoints: EndpointDetail[],
  opts?: { maxPayloads?: number; maxBodyFields?: number; maxParamPayloads?: number }
): HeaderInjectionTest[] {
  const tests: HeaderInjectionTest[] = [];
  ...
  const payloads = HEADER_PAYLOADS.slice(0, maxPayloads);
  ...
  for (const ep of endpoints) {
    ...
    const bodyPaths = findStringPaths(base.body)
      .filter((p) => HEADER_LIKE_PARAM.test(p))
    ...
    for (const param of base.queryParams.filter((p) => HEADER_LIKE_PARAM.test(p))) {
    ...
    for (const param of base.pathParams.filter((p) => HEADER_LIKE_PARAM.test(p))) {
      for (const payload of payloads.slice(0, maxParamPayloads)) {
        tests.push({
          method: ep.method,
    ...
    for (const param of base.headerParams.filter((p) => HEADER_LIKE_PARAM.test(p))) {
      for (const payload of payloads.slice(0, maxParamPayloads)) {
        tests.push({
          method: ep.method,
          path: base.resolvedPath,
    ...
    for (const param of base.cookieParams.filter((p) => HEADER_LIKE_PARAM.test(p))) {
      for (const payload of payloads.slice(0, maxParamPayloads)) {
        tests.push({
          method: ep.method,
          path: base.resolvedPath,
