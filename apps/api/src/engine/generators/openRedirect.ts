...
const REDIRECT_PAYLOADS = [
  "https://evil.example.com/tomsec-open-redirect",
  "//evil.example.com/tomsec-open-redirect"
];

const URL_LIKE_PARAM = /(url|uri|redirect|return|next|target|dest|destination|callback|continue)/i;

export function generateOpenRedirectTests(
  endpoints: EndpointDetail[],
  opts?: { maxPayloads?: number; maxBodyFields?: number; maxParamPayloads?: number }
): OpenRedirectTest[] {
  const tests: OpenRedirectTest[] = [];
  const maxPayloads = opts?.maxPayloads ?? 2;
  const maxBodyFields = opts?.maxBodyFields ?? 6;
  const maxParamPayloads = opts?.maxParamPayloads ?? 2;
  const payloads = REDIRECT_PAYLOADS.slice(0, maxPayloads);
  ...
  for (const ep of endpoints) {
    ...
    const bodyPaths = findStringPaths(base.body)
      .filter((p) => URL_LIKE_PARAM.test(p))
    ...
    for (const param of base.queryParams.filter((p) => URL_LIKE_PARAM.test(p))) {
    ...
    for (const param of base.pathParams.filter((p) => URL_LIKE_PARAM.test(p))) {
      for (const payload of payloads.slice(0, maxParamPayloads)) {
    ...
    for (const param of base.headerParams.filter((p) => URL_LIKE_PARAM.test(p))) {
      for (const payload of payloads.slice(0, maxParamPayloads)) {
        tests.push({
          method: ep.method,
          path: base.resolvedPath,
    ...
    for (const param of base.cookieParams.filter((p) => URL_LIKE_PARAM.test(p))) {
      for (const payload of payloads.slice(0, maxParamPayloads)) {
        tests.push({
          method: ep.method,
          path: base.resolvedPath,
...
function buildExampleFromSchema(schema: any, depth: number): any {
  ...
  if (type === "string") return schema.format === "uri" ? "https://example.com" : "test";
