...
const PATH_LIKE_PARAM = /(path|file|filename|dir|directory|folder|resource|name|target|document)/i;

export function generatePathTraversalTests(
  endpoints: EndpointDetail[],
  opts?: { maxPayloads?: number; maxBodyFields?: number; maxParamPayloads?: number }
): PathTraversalTest[] {
  const tests: PathTraversalTest[] = [];
  ...
  const payloads = TRAVERSAL_PAYLOADS.slice(0, maxPayloads);
  ...
  for (const ep of endpoints) {
    ...
    const bodyPaths = findStringPaths(base.body)
      .filter((p) => PATH_LIKE_PARAM.test(p))
    ...
    for (const param of base.queryParams.filter((p) => PATH_LIKE_PARAM.test(p))) {
    ...
    for (const param of base.pathParams.filter((p) => PATH_LIKE_PARAM.test(p))) {
      for (const payload of payloads.slice(0, maxParamPayloads)) {
        tests.push({
          method: ep.method,
    ...
    for (const param of base.headerParams.filter((p) => PATH_LIKE_PARAM.test(p))) {
      for (const payload of payloads.slice(0, maxParamPayloads)) {
        tests.push({
          method: ep.method,
          path: base.resolvedPath,
    ...
    for (const param of base.cookieParams.filter((p) => PATH_LIKE_PARAM.test(p))) {
      for (const payload of payloads.slice(0, maxParamPayloads)) {
        tests.push({
          method: ep.method,
          path: base.resolvedPath,
